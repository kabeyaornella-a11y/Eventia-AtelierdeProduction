...
import { ecrinsModels, seuilsModels, voilesModels, unionModels, cloudinaryPoster } from "@/lib/cloudinary-models";
import { ExperienceLightbox, type LightboxExperience } from "@/components/site/premium/ExperienceLightbox";
...
const pillars: Record<Collection["slug"], { title: string; lines: string[]; palette: string[]; mood: string }> = {
  ecrins: {
...
export function CollectionPage({ slug }: { slug: Collection["slug"] }) {
  ...
  const heroExp: LightboxExperience = exps[0]
    ? { slug: exps[0].slug, name: c.name, story: c.manifesto, image: c.image, univers: slug, priceFrom: exps[0].priceFrom, video: firstModel?.video }
    : { slug: "", name: c.name, story: c.manifesto, image: c.image, univers: slug, video: firstModel?.video };
  ...
  return (
    <SiteLayout>
      ...
      <section className="relative h-[80vh] min-h-[560px] overflow-hidden">
        {firstModel ? (
          <video src={firstModel.video} poster={cloudinaryPoster(firstModel.video)} autoPlay muted loop playsInline preload="metadata" className="absolute inset-0 w-full h-full object-cover" />
        ) : (
        ...
        <div className="absolute inset-0 flex items-end">
          <div className="mx-auto max-w-7xl w-full px-6 lg:px-10 pb-16 text-ivory">
            ...
            <div className="font-serif-soft italic text-xl md:text-2xl text-ivory/90 mt-4">{c.tagline}</div>
            <button
          ...
          <div className="grid lg:grid-cols-2 gap-14 items-center">
            <div className="space-y-6">
              ...
              <ul className="space-y-4 font-serif-soft italic text-lg text-muted-foreground">
                {p.lines.map((l) => (
                  <li key={l} className="pl-6 border-l-2 border-primary/40">{l}</li>
                ))}
              </ul>
            </div>
            <div className="space-y-6">
              ...
              <div className="grid grid-cols-5 gap-3">
                {p.palette.map((h) => (
                  <div key={h} className="aspect-square shadow-soft" style={{ backgroundColor: h }} />
                ))}
              </div>
              <div className="grid grid-cols-2 gap-4 pt-6">
                <div className="bg-background p-5 shadow-soft">
                  ...
                  <div className="font-serif-soft italic text-sm text-muted-foreground">Cormorant Garamond</div>
                </div>
                <div className="bg-background p-5 shadow-soft">
                  <div className="eyebrow text-[10px] mb-2">Atmosphère</div>
                  <div className="font-serif-soft italic text-base text-foreground/85">{p.mood}</div>
                </div>
              </div>
            </div>
          </div>
      ...
      <ModelGrid eyebrow={collectionModels[slug].label} title="Les modèles de la collection" models={collectionModels[slug].models} />
      ...
      <div className="bg-cacao text-ivory">
        <Section className="text-center">
          ...
          <h2 className="font-display text-4xl md:text-5xl">Composez votre expérience dans {c.name}</h2>
          ...
          <div className="flex flex-wrap justify-center gap-4 mt-10">
            <Link to="/configurateur"><GoldButton>Composer dans {c.name}</GoldButton></Link>
            <Link to="/experiences">
