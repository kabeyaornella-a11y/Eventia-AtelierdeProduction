...
export const Route = createFileRoute("/contact")({
  head: () => ({
    meta: [
      ...
      { name: "description", content: "Contactez Eventia Signature pour créer une expérience digitale premium : mariage, save the date, RSVP, album photo live et demande sur mesure." },
      { name: "keywords", content: "contact invitation digitale mariage, devis wedding website luxe, RSVP mariage premium, Eventia Signature" },
...
function ContactPage() {
  ...
  const [form, setForm] = useState({ nom: "", email: "", tel: "", date: "", type: "Mariage", message: "" });
    ...
    try {
      await submitLead({
        data: {
          ...
          message: [form.date ? `Date envisagée : ${form.date}` : "", form.message].filter(Boolean).join("\n\n") || undefined,
        },
      });
      ...
      <section className="relative h-[60vh] min-h-[420px] overflow-hidden">
        ...
        <div className="absolute inset-0 flex items-end">
          <div className="mx-auto max-w-7xl w-full px-6 lg:px-10 pb-14 text-ivory">
            ...
            <h1 className="font-display text-5xl md:text-7xl leading-[1.05] max-w-3xl">Parlons de votre événement.</h1>
      ...
      <Section>
        <div className="grid lg:grid-cols-[1.3fr_1fr] gap-14">
          ...
          <div className="bg-ivory p-8 lg:p-12 shadow-soft">
            ...
            <p className="text-muted-foreground mb-8">Nous vous répondons personnellement sous 24 heures ouvrées.</p>
            <form
              onSubmit={handleSubmit}
              className="space-y-4"
            >
              <div className="grid sm:grid-cols-2 gap-4">
                <Field label="Prénom & Nom" required value={form.nom} onChange={(v) => setForm({ ...form, nom: v })} />
                <Field label="Email" type="email" required value={form.email} onChange={(v) => setForm({ ...form, email: v })} />
                <Field label="Téléphone" type="tel" value={form.tel} onChange={(v) => setForm({ ...form, tel: v })} />
                <Field label="Date envisagée" type="date" value={form.date} onChange={(v) => setForm({ ...form, date: v })} />
              </div>
              <div>
                ...
                >
                  {["Mariage", "Fiançailles", "Naissance", "Baptême", "Anniversaire", "Sur mesure"].map((t) => (
              ...
              <GoldButton type="submit" disabled={loading} className="w-full md:w-auto">{loading ? "Envoi en cours" : "Envoyer ma demande"}</GoldButton>
            </form>
          </div>
          ...
          <div className="space-y-6">
            <div className="bg-cacao text-ivory p-8 shadow-gold">
              ...
              <p className="font-serif-soft italic text-ivory/85 mt-3 leading-relaxed">
                Chaque demande est étudiée par notre direction artistique. Réponse personnelle, jamais automatique.
              </p>
            </div>

            <div className="bg-ivory shadow-soft p-6 space-y-5">
              <Info icon={<Mail className="size-5 text-primary" />} label="Par email" value="bonjour@eventiasignature.com" />
              <Info icon={<Phone className="size-5 text-primary" />} label="Par téléphone" value="+33 (0)1 00 00 00 00" />
              <Info icon={<MapPin className="size-5 text-primary" />} label="Adresse" value="Paris · France" />
              <Info icon={<Clock className="size-5 text-primary" />} label="Horaires" value="Lun · Ven 10h. 19h" />
            </div>

            <div className="bg-ivory shadow-soft p-6">
              <div className="eyebrow text-[10px] mb-2">Wedding planners</div>
              <p className="text-sm text-foreground/80">Programme partenaires dédié. Conditions avantageuses sur demande.</p>
            </div>
          </div>
        </div>
      </Section>
...
function Field({ label, value, onChange, type = "text", required }: { label: string; value: string; onChange: (v: string) => void; type?: string; required?: boolean }) {
  return (
    <div>
      <label className="eyebrow text-[10px] mb-1.5 block">{label}{required && " *"}</label>
      <input
        type={type}
        required={required}
        value={value}
