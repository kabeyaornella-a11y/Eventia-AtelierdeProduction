  ...
  head: () => ({
    meta: [
      { title: "Espace client — Eventia Signature" },
      {
        name: "description",
        content:
          "Accédez à votre espace client Eventia Signature pour retrouver vos commandes et vos compositions.",
      },
...
function translateAuthError(message: string): string {
  const map: Record<string, string> = {
    ...
    "Email not confirmed":
      "Veuillez confirmer votre adresse e-mail avant de vous connecter (pensez à vérifier vos spams).",
...
function AuthPage() {
  ...
  const submit = async (e: React.FormEvent) => {
    ...
    try {
      ...
      toast.error(
        err instanceof Error ? translateAuthError(err.message) : "Erreur d'authentification",
      );
    } finally {
      setBusy(false);
    }
    ...
    <SiteLayout>
      <Section className="max-w-md mx-auto">
        <div className="text-center">
          ...
          <h1 id="auth-form-title" className="font-display text-4xl md:text-5xl mt-3">
            {mode === "signin" ? "Votre espace" : "Créer un compte"}
          </h1>
        ...
        >
          <div>
            <label htmlFor="auth-email" className="eyebrow text-xs">
              Email
            </label>
            <input
              id="auth-email"
          ...
          <div>
            <label htmlFor="auth-password" className="eyebrow text-xs">
              Mot de passe
            </label>
            <input
              id="auth-password"
            ...
            {mode === "signup" && (
              <div className="text-[11px] text-muted-foreground mt-1">
                Min. 8 caractères. Les mots de passe compromis sont refusés.
              </div>
            )}
            {mode === "signin" && (
