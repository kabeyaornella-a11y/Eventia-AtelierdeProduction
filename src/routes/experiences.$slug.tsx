  ...
  head: ({ params }) => {
    ...
    return {
      meta: [
        ...
        { name: "keywords", content: `${e?.name ?? "expérience digitale"}, invitation digitale mariage premium, wedding website luxe, Eventia Signature` },
    ...
    <SiteLayout>
      <Section>
        <div className="text-center">
          ...
          <Link to="/experiences" className="mt-6 inline-block text-primary">← Toutes les expériences</Link>
        </div>
      </Section>
    </SiteLayout>
  ),
...
function ExperiencePage() {
  ...
  const related = experiences.filter((x) => x.univers === e.univers && x.slug !== e.slug).slice(0, 3);
  ...
  return (
    <SiteLayout>
      ...
      <section className="relative h-[78vh] min-h-[540px] overflow-hidden">
        {video ? (
          <video src={video.video} poster={cloudinaryPoster(video.video)} autoPlay muted loop playsInline preload="metadata" className="absolute inset-0 w-full h-full object-cover" />
        ) : (
        ...
        <div className="absolute inset-0 flex items-end">
          <div className="mx-auto max-w-7xl w-full px-6 lg:px-10 pb-16 text-ivory">
            <div className="eyebrow !text-primary-soft mb-3">{c.name} · {e.immersion}</div>
        ...
        <div className="grid md:grid-cols-[0.9fr_1.1fr] gap-10 items-center">
          <div className="bg-ivory border border-border p-8 shadow-soft">
            ...
            <p className="font-serif-soft italic text-lg text-muted-foreground mt-5 leading-relaxed">
              Une ouverture immersive, une révélation des prénoms, une date mise en scène, les informations essentielles, puis une réponse RSVP fluide.
            </p>
          </div>
          <div className="grid gap-3 text-sm">
            {["Ouverture cinématographique", "Révélation du couple", "Date et compte à rebours", "Lieu, programme et itinéraire", "RSVP premium", "Remerciement final"].map((step, i) => (
              <div key={step} className="flex items-center gap-4 bg-background border border-border px-5 py-4">
            ...
            <div className="space-y-4">
              ...
              <div className="flex gap-2 flex-wrap">
                {e.palette.map((h: string) => (
                  <div key={h} className="size-14 rounded-full border border-border shadow-soft" style={{ backgroundColor: h }} />
                ))}
              </div>
            </div>
            <div className="space-y-3">
              <div className="eyebrow">Inspiration</div>
              <p className="font-serif-soft italic text-lg text-foreground/85 leading-relaxed">{e.inspiration}</p>
            </div>
            <div className="space-y-3">
              <div className="eyebrow">Musique</div>
              <p className="font-serif-soft italic text-lg text-foreground/85 leading-relaxed">{e.music}</p>
            </div>
          </div>
        </Section>
      </div>
        ...
        <div className="grid md:grid-cols-3 gap-4">
          {gallery.map((src: string, i: number) => (
            <div key={i} className="aspect-[4/5] overflow-hidden shadow-soft">
              <img src={src} alt={`${e.name}. vue ${i + 1}`} loading="lazy" className="w-full h-full object-cover hover:scale-105 transition-transform duration-700" />
            </div>
          ))}
        </div>
      </Section>
      ...
      <div className="bg-cacao text-ivory">
        <Section className="text-center">
          ...
          <p className="font-serif-soft italic text-lg text-ivory/80 mt-4">À partir de <span className="text-primary-soft">{e.priceFrom} €</span>. paiement unique, sans abonnement.</p>
          <div className="flex flex-wrap justify-center gap-4 mt-10">
            <Link to="/configurateur"><GoldButton>Composer cette expérience</GoldButton></Link>
            <Link to="/contact">
      ...
      {related.length > 0 && (
        <Section>
          ...
          <div className="grid sm:grid-cols-3 gap-6">
            {related.map((r) => (
              <Link key={r.slug} to="/experiences/$slug" params={{ slug: r.slug }} className="group block bg-ivory shadow-soft overflow-hidden">
                <div className="aspect-[4/3] overflow-hidden">
                  {findModelByName(r.name) ? (
                    <video src={findModelByName(r.name)!.video} muted loop playsInline autoPlay preload="metadata" className="w-full h-full object-cover transition-transform duration-700 group-hover:scale-105" />
                  ) : (
                ...
                <div className="p-5">
                  ...
                  <div className="font-serif-soft italic text-sm text-muted-foreground mt-1">À partir de {r.priceFrom} €</div>
                </div>
              </Link>
            ))}
          </div>
